...
import com.telekom.api.sendsms.model.SendSmsRetrieveRequest;

/**
 * Wrapper for Telekom Send SMS service
 */
public class SendSmsReceiveClient extends TelekomClient {
	 ...
	 */
	public SendSmsReceiveClient(TelekomAuth authentication,
	...
	private TelekomJsonWebRequest createAuthenticatedJsonRequest(String uri,
			SendSmsRetrieveRequest request) throws JsonGenerationException,
	 ...
	 */
	public SmsResponse sendSmsRetrieve(SendSmsRetrieveRequest request) throws IOException {
		ensureRequestValid(request);

		String uri = getServiceBaseUrl() + "/inbound/registrations/%s/messages";
		
		uri = String.format(uri, urlEncode(request.getRegistrationId()));
